...
"""
AIC2026 Exp04 modality ablation: RGB + IR.
...
EXPERIMENT_NAME = "exp04_ablation_rgbi_early4_yolo11s_960"
...
def check_environment() -> None:
    section("RGBI 4ch Formal Training - Preflight")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA is unavailable. Formal RGBI training should not start on CPU."
        )
...
def verify_best_checkpoint(trainer: MultimodalDetectionTrainer) -> None:
    section("RGBI 4ch Post-training Verification")
    ...
    ir_weight = weight[:, 3]
    ir_nonzero = torch.count_nonzero(ir_weight).item()

    print("IR stem nonzero =", ir_nonzero)
    print("IR stem abs sum =", ir_weight.abs().sum().item())

    if ir_nonzero == 0:
        raise AssertionError("IR stem remained zero after training.")

    print("[PASS] best.pt 4-channel RGBI")
    print("[PASS] best.pt 12-class")
    print("[PASS] learned IR stem")
...
def main() -> None:
    ...
    section("Build RGBI 4ch Formal Trainer")
